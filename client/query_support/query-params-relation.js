...
class QueryParamsAggregateUndirectedGraph {
      ...
      table.forEach((value, key) => {
         ...
         mutualExclusiveList.forEach((obj, idx) => {
            ...
            if (obj.optionKey === flags.ALL_OTHER_SORT) {
               ...
               for (let key of Options.sortMap.keys()) {
               ...
               for (let key of Options.filterMap.keys()) {
                  if (key === optionKey) return;
   ...
   setAllCoexistedFilterRelationAtSortParamNodes() {
      this.coexistedFilterRelation = new Map();  /* {sortOptionIdx: {filterOptionIdx: coexistedFilterOptionIdx}} */
      for (let sortKey of Options.sortMap.keys()) {
         const sortIdx = this.entries[sortKey];

         const coexistFilterMap = this.matrix[sortIdx]
            .reduce((arr, value, idx) => this.getNeighborNodesCallback(sortIdx, arr, value, idx), [])
            .reduce((acc, node_idx, _, array) => {
               ...
               const coexistFilters = this.matrix[node_idx]
                  ...
                  .filter((filter_idx) => array.some((neighbor_node_idx) => filter_idx === neighbor_node_idx));
   ...
   pruneQueryParams(query) {
      ...
      const params = Object.entries(query).reduce((acc, [key, value]) => {
         ...
         return overriddenOptions.reduce((acc, option) =>
            typeof query[option['optionKey']] === 'undefined' ? /* prevent duplicate options */
               { ...acc, [option['optionKey']]: option['optionValue'] } : acc
            , { ...acc, [key]: value });
      ...
      const visitedList = [];
      for (let counter = 0;
         Object.keys(params).length !== visitedList.reduce((acc, obj) =>
         ...
         visitedList.push({});
         const residuals = Object.keys(params).filter((value) => typeof visitedGroup[value] === 'undefined');

         this.dfs(params, visitedList[counter], visitedGroup, null, this.entries[residuals[0]]);
      }

      //select the query parameters formation that is the most similar to original query parameters
      const acceptedParams = visitedList.reduce((max, obj, idx) => {
         const priority = Object.keys(obj).reduce((acc, optionKey) => {
            return acc + optionMap.get(optionKey).priority;
         }, 0);
         const [update_max, max_idx] = priority > max[1] ? [priority, idx] : [max[1], max[2]];
      ...
      }, [{}, 0, 0]);   /* [acceptedParamsObj, max_priority_total, max_idx] */
   ...
   getNeighborsAfterFilterCoexistenceTest(selfObj, isPrevSortNode, prevNodeIdx) {
      ...
      const coexistFilterOptionMap = selfObj.coexistedFilterRelation.get(prevNodeIdx);
      const list = this.map((node_idx, _, array) => {
         const res = coexistFilterOptionMap.get(node_idx).filter((value) => array.some(((val) => val === value)));
         res.push(node_idx);
         return res;
      })
   ...
   dfs(params, visited, visitedGroup, prev, next) {
      ...
      let neighbors = this.matrix[next]
         .reduce((arr, value, idx) => this.getNeighborNodesCallback(next, arr, value, idx), [])
         .filter((value) => typeof params[this.vertices[value]] !== 'undefined')
         .filter((value) => typeof visitedGroup[this.vertices[value]] === 'undefined')
         .filter((value) => typeof visited[this.vertices[value]] === 'undefined');

      if (isCurrentSortNode && prev !== null) {
         //prev node may cause conflict
         const coexistFilterOptionsWithPrevFilter = this.coexistedFilterRelation.get(next).get(prev);
         neighbors = coexistFilterOptionsWithPrevFilter.filter((value) => neighbors.some((val) => value === val));
      } else {
         neighbors = this.getNeighborsAfterFilterCoexistenceTest.apply(neighbors, [this, isCurrentSortNode, next]);
      }
      prev = next;
